...
const expoVersion = pkg.dependencies && pkg.dependencies.expo

let sdkVersion = ''
if (expoVersion) {
  const major = expoVersion.match(/\d+/)?.[0]
  if (major) {
    const candidate = `${major}.0.0`
...
async function uploadToSnack() {
  ...
  // Add expo-router/head dependency explicitly for snack
  snackDependencies['expo-router/head'] = {version: '~3.5.24'}

  const nodeEnv = process.env.NODE_ENV

  console.debug('uploadToSnack -> process.env.NODE_ENV:', nodeEnv)
